...
sender_address = web3.toChecksumAddress('0x662F4C7964bAf75b1ac1527d0e8B91B63e85bCb6') #the address which buys the token
tokenToBuy = web3.toChecksumAddress('0x62858686119135cc00C4A3102b436a0eB314D402')



read_balance = web3.fromWei(web3.eth.get_balance(sender_address),'ether')
print(read_balance)
...
def buy():
    ...
    60000, # set to 0, or specify minimum amount of token you want to receive - consider decimals!!!
    [spend,tokenToBuy],
    sender_address,
    (int(time.time()) + 10000)
    ).buildTransaction({
    'from': sender_address,
    'value': web3.toWei(0.5,'ether'),#This is the Token(BNB) amount you want to Swap from
    'gasPrice': web3.toWei('30','gwei'),
    'nonce': nonce,
    })

    signed_txn = web3.eth.account.sign_transaction(pancakeswap2_txn, private_key=file1.private)
...
def handle_event(event):
    ...
    global blocknum
    blocknum = int(Web3.toJSON(event['blockNumber']))
...
def wait_for_buy(block_time):
    while web3.eth.block_number < block_time + 4:
        print(web3.eth.block_number)
    new_balance = read_balance
    while new_balance == read_balance:
        try:
            buy()
        except:
            ValueError
        ...
        print("attempt:" ,time)
        new_balance = web3.fromWei(web3.eth.get_balance(sender_address),'ether')
    print("UR BALANCE DROPPED BUT THAT DONT MEAN THE TRANSACTION WENT THROUGH BSC BLOCKCHAIN MIGHT OF CRASHED G!")
...
def log_loop(event_filter, poll_interval):
    ...
    while count == 0:
        for PairCreated in event_filter.get_new_entries():
            if handle_event(PairCreated) == True:
                print("success")
                count +=1
    wait_for_buy(blocknum)
...
def main():
    ...
    log_loop(event_filter,0.001)
                # log_loop(block_filter, 2),
                # log_loop(tx_filter, 2)))
        
        # close loop to free up system resources
     
    print("shuch ur mouth")
